...
db_dir = "jma_update"  # DBディレクトリ
path = os.path.join(db_dir, "jma.db")  # DBのパス/ファイル名 = "jma_update/jma.db"
...
def init_db():
    ...
    with sqlite3.connect(path) as con:  # DBに接続・処理が終了したら自動的にDBを閉じる
        cur = con.cursor()  # SQLを実行するためのカーソルを取得
    
        # テーブルの作成
        sql = '''CREATE TABLE IF NOT EXISTS weather_data (
            id INTEGER PRIMARY KEY AUTOINCREMENT,   -- ID(自動的に割り当てられる)
            area_code TEXT,             -- 地域コード
            area_name TEXT,             -- 地域名
            timestamp DATETIME ,        -- 時刻
            temperature_max REAL,       -- 最高気温
            temperature_min REAL,       -- 最低気温
            weather_condition TEXT,     -- 天気
            UNIQUE(area_code, timestamp)   -- 同じ場所における全く同じ時間の重複を防ぐ
            )'''
...
def main(page: ft.Page):
    ...
    # 天気情報を保存する関数
    def save_weather(weather_data_list):
        try:
            with sqlite3.connect(path) as con:
                cur = con.cursor()
                # データを挿入するSQL(カラム名を指定)
                sql_insert = '''INSERT INTO weather_data 
                    (area_code, area_name, timestamp, temperature_max, temperature_min, weather_condition)
                    VALUES (?, ?, ?, ?, ?, ?)'''
                
                cur.executemany(sql_insert, (weather_data_list))  # 複数のデータを一度に挿入
                con.commit()
        except sqlite3.IntegrityError:  # 重複エラー時の処理
            print(f"同じデータが既に存在します")
    ...
    def update_weather(e, area_id):
        try:
            ...
            for i in range(7):   # 天気情報は7日分取得(固定)
                try:
                    ...
                    weather_data_list.append((
                        area_id,                                   # area_code
                        areas_data["offices"][area_id]["name"],    # area_name
                        formatted_date,                            # timestamp
                        max_temp if max_temp != "--" else None,    # temperature_max
                        min_temp if min_temp != "--" else None,    # temperature_min
                        weather_codes.get(weather, weather)        # weather_condition
                    ))
            ...
            # 7日分のデータをまとめて保存
            save_weather(weather_data_list)
